                 ...
                 std::vector<int> &ops)
{
    ...
    // RECOVERY PHASE-------------------------------------------------------------------------------------
    auto func1 = [idx, &init_keys, num_thread, &values, index_type](uint64_t thread_id, bool)
    {
        size_t total_num_key = init_keys.size();
    ...
    auto func2 = [idx, &init_keys, num_thread, &values, index_type](uint64_t thread_id, bool)
    {
        size_t total_num_key = init_keys.size();
        size_t key_per_thread = total_num_key / num_thread;
        size_t start_index = key_per_thread * thread_id;
    ...
    // CACHE WARM-UP--------------------------------------------------------------------------------------
#if 1
    auto func3 = [idx, &init_keys, num_thread, &values, index_type](uint64_t thread_id, bool)
    {
        size_t total_num_key = init_keys.size();
        size_t key_per_thread = total_num_key / num_thread;
        size_t start_index = key_per_thread * thread_id;
