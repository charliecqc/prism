...
bool ValueStorage::read_gc_r_chunk(int gc_r_chunk_offset)
{
    ...
    if (ret != submitted_io)
    {
        ...
        ts_trace(TS_INFO, "[GC-READ] io_uring_submit successed! | ret: %d i: %d\n", ret, i);
    if (ret < 0)
    {
    ...
    if (ret < 0)
    {
        ...
        ts_trace(TS_INFO, "io_uring_wait_cqe successed!\n");
...
void ValueStorage::write_chunk(w_chunk_t *chunk, bool write_type)
{
    ...
    if (ret < 0)
    {
        ...
        ts_trace(TS_INFO, "io_uring_submit successed! | ret: %d i: %d\n", ret, i);
...
void ValueStorage::write_gc_w_chunk(int gc_w_chunk_offset)
{
    ...
    if (ret != submitted_io)
    {
        ...
        ts_trace(TS_INFO, "[GC-WRITE] io_uring_submit successed! | ret: %d i: %d\n", ret, i);
    if (ret < 0)
    {
